...
public class ProjectUtils {
    ...
    public static double getTotalInvoice( Map<ElectricDevice, Integer> items, int invoiceType) {
        double total=0;
        for (Map.Entry<ElectricDevice, Integer> entry : items.entrySet()){
            double price;
            if (invoiceType==Constant.IMPORT_INVOICE){
                price=entry.getKey().getImportPrice();
            }else {
                price=entry.getKey().getSalePrice();
            }
    ...
    public static int getInputInteger(String property) {
        while (true) {
            System.out.println("Enter " + property + ":");
            try {
